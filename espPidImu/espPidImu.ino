...
// MPU6050 variables
MPU6050 mpu;
float yaw = 0; // Robot's orientation
float x = 0, y = 0; // Robot's position
...
// Calculate position using MPU6050 and encoders
void calculatePosition() {
  ...
  // Calculate yaw (orientation)
  yaw += gz * dt / 131.0; // 131 is the sensitivity factor for +/- 250 deg/s range

  // Calculate distance traveled by each wheel
  float wheelCircumference = 0.2; // Wheel circumference in meters (adjust as needed)
  float encoderResolution = 1000; // Encoder ticks per revolution (adjust as needed)
  float distanceA = (counterA / encoderResolution) * wheelCircumference;
  float distanceB = (counterB / encoderResolution) * wheelCircumference;

  // Calculate average distance and change in orientation
  float distance = (distanceA + distanceB) / 2;
  float deltaYaw = (distanceA - distanceB) / 0.3; // 0.3 is the wheelbase (adjust as needed)
